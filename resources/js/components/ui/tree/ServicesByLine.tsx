...
export default function ServicesByLine({ line, services, selected, onToggle }: ServicesByLineProps) {
    return (
        <div className="mt-10">
            ...
            <p className="mb-8 text-center text-slate-600">Selecciona uno o varios servicios</p>
            {services.length > 0 ? (
                        ...
                        return (
                            <Card
                                ...
                                className={`card-shine hover-lift group cursor-pointer overflow-hidden rounded-2xl border-2 transition-all duration-300 animate-slide-up ${staggerClass} ${
                                    isSelected
                                        ? 'border-[#0693e3] bg-gradient-to-br from-[#0693e3]/8 to-white shadow-lg shadow-[#0693e3]/20'
                                        : 'border-slate-200 bg-white hover:border-[#0693e3]/45 hover:shadow-lg'
                                }`}
                            >
                                <CardContent className="flex min-h-[6rem] items-center gap-4 p-5">
                                    <div
                                    ...
                                    <p className="min-w-0 flex-1 text-left text-base font-medium text-slate-900">{service}</p>

                                    {isSelected && <CheckCircle2 className="h-6 w-6 shrink-0 text-[#0693e3]" aria-hidden />}
                                </CardContent>
                            </Card>
                        );
                    })}
