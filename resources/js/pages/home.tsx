    ...
    return (
        <div className="min-h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-white to-slate-50">
            ...
            {showSuccess && successMessage && (
                <div className="animate-fade-in fixed left-1/2 top-6 z-50 w-full max-w-md -translate-x-1/2 rounded-lg bg-green-100 px-4 py-3 text-green-800 shadow-lg">
                    {successMessage}
                    <button className="ml-4 text-xs text-green-700 underline" onClick={() => setShowSuccess(false)}>
                        Cerrar
                    </button>
                </div>
            )}

            <main className="flex min-h-screen items-center justify-center pb-20 pt-20">
