...
const Cotizar: React.FC<CotizarProps> = ({ viewData }) => {
  ...
  const prepareBackData = () => {
    ...
    Object.entries(responses).forEach(([key, value]) => {
      if (key === "services") {
        ...
        backData["sedes"] = Array.isArray(value)
          ? value.filter((sede: string) => sede.trim() !== "")
          : [value];
      } else {
  ...
  const sendDataToBackend = async (action: "saveToList" | "quoteNow") => {
    try {
      ...
      if (action === "quoteNow") {
        window.location.href = `/portfolio?backData=${encodeURIComponent(
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
                      ...
                      <div className="flex flex-col">
                        <button
                          ...
                          className={`p-4 rounded-lg border transition-all flex items-center justify-center mb-2 ${
                            otherInputs[sectionName] ||
                            (responses[sectionName] && responses[sectionName].isOther)
            ...
            <div className="flex justify-between mt-8 pt-6 border-t border-gray-200">
              <button
                type="button"
                onClick={() => sendDataToBackend("saveToList")}
              ...
              <button
                type="button"
                onClick={() => sendDataToBackend("quoteNow")}
          ...
          <div className="bg-blue-50 rounded-xl p-6 border border-blue-100">
            ...
            <p className="text-blue-700 text-sm">
              Una vez que envíe su solicitud, nos contactaremos con usted dentro de las próximas 24
              horas para proporcionarle una cotización detallada según sus requerimientos.
            </p>
          </div>
        </div>
      </form>
